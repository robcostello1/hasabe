...
function useDatabaseMetadata() {
  ...
  const setupMultiLeaderSync = (srcDatabase: any) => {
    srcDatabase.replicate
      .from(DatabaseEnums.ExternalApi)
      .on("complete", function (info: any) {
        // then two-way, continuous, retriable sync
        srcDatabase
          .sync(DatabaseEnums.ExternalApi, {
            live: true,
            retry: true,
          })
          .on("change", () => {})
  ...
  const fetchMetadata = async () => {
    let info: any;
    try {
      info = await metadataDatabaseInstance.get(
        DatabaseEnums.MetadataDBLookupRow
      );
    } catch (err) {
